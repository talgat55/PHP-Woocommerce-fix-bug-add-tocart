...
class WC_Product_Data_Store_CPT extends WC_Data_Store_WP implements WC_Object_Data_Store_Interface, WC_Product_Data_Store_Interface {
	...
	*/
	 ...
	 */
		public function find_matching_product_variation1( $product, $match_attributes = array() ) {
		$query_args = array(
		...
		if ( $variations && ! is_wp_error( $variations ) ) {
			 ...
			 */
			var_dump(( array_map( 'sanitize_title', $match_attributes ) === $match_attributes ) ? 0 : $this->find_matching_product_variation( $product, array_map( 'sanitize_title', $match_attributes ) ));exit;
...
exit;
	...
	public function find_matching_product_variation( $product, $match_attributes = array() ) {
		...
		$variation_ids = $wpdb->get_results("
			...
			WHERE post_parent  = " . $product->get_id() . "
		...
		//var_dump($matched_variation);exit;
